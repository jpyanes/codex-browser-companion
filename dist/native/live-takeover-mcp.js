...
// src/native/live-takeover-mcp.ts
...
async function fetchLiveTakeoverState(endpoint = DEFAULT_LIVE_TAKEOVER_ENDPOINT, options = {}) {
  try {
    const health = await fetchJson(`${endpoint}/health`, LIVE_TAKEOVER_REQUEST_TIMEOUT_MS);
...
async function enqueueLiveTakeoverCommand(endpoint = DEFAULT_LIVE_TAKEOVER_ENDPOINT, command) {
...
async function getLiveTakeoverResult(endpoint = DEFAULT_LIVE_TAKEOVER_ENDPOINT, commandId) {
...
function resolveTabContext(args) {
...
async function waitForLiveTakeoverResult(endpoint, commandId, timeoutMs = DEFAULT_TIMEOUT_MS) {
...
async function queueAndWait(endpoint, command, timeoutMs = DEFAULT_TIMEOUT_MS) {
  const liveEndpoint = endpoint ?? DEFAULT_LIVE_TAKEOVER_ENDPOINT;
        ...
        JSON.stringify(
          {
            ...
            summary: state.enabled ? state.status === "connected" ? `Attached to ${state.activeTitle || state.activeUrl || `tab ${state.activeTabId ?? "unknown"}`}.` : state.status === "connecting" ? "Waiting for the visible Chrome tab to heartbeat." : `Live takeover is ${state.status}.` : "Live takeover mode is disabled."
          },
          null,
          2
        )
...
server.tool(
  ...
  "Capture a structured snapshot of the current visible tab through live takeover.",
  {
    ...
    mode: external_exports.enum(["full", "interactive", "summary", "suggestions"]).optional(),
...
server.tool(
  ...
  "Click an element in the current visible tab through live takeover.",
  {
    ...
    bridgeId: external_exports.string().optional(),
...
server.tool(
  ...
  "Type into a field in the current visible tab through live takeover.",
  {
    ...
    clearBeforeTyping: external_exports.boolean().optional(),
    timeoutMs: external_exports.number().int().min(1e3).max(12e4).optional()
  },
...
await main();
//# sourceMappingURL=live-takeover-mcp.js.map
