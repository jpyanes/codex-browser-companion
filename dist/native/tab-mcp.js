...
function buildPageStateSummary(pageState) {
...
// src/shared/native-tab-context.ts
...
function normalizeNumber2(value, fallback) {
...
function normalizeNativeBrowserTabSummary(value) {
  ...
  const readyState = candidate.readyState === "loading" || candidate.readyState === "interactive" || candidate.readyState === "complete" ? candidate.readyState : "complete";
  return {
    tabId: normalizeNumber2(candidate.tabId, -1),
    browserTargetId,
    url,
    title,
    readyState,
    interactiveCount: normalizeNumber2(candidate.interactiveCount, 0),
    formCount: normalizeNumber2(candidate.formCount, 0),
    visibleTextLength: normalizeNumber2(candidate.visibleTextLength, 0),
...
function inferNativePageKind(summary) {
  ...
  if (summary.hasSensitiveInputs || hostname === "accounts.google.com" || /(sign in|log in|login|authenticate|verify account)/.test(haystack)) {
    return "login";
  }
  if (/(checkout|payment|pay now|pay|billing|card|credit card|purchase|buy now|place order|order summary|subscription|donate|invoice|wallet|paypal)/.test(
    haystack
  )) {
    return "payment";
  }
  ...
  if (summary.visibleTextLength > 3e3) {
    return "article";
  }
...
function inferNativeNavigationMode(summary, pageKind) {
  if (pageKind === "spa") {
    return "spa";
  }
  const hostname = hostnameFromUrl(summary.url);
...
function buildNativePageState(summary) {
...
function buildNativeTabContext(summary) {
...
function buildNativeTrackedTab(summary) {
  const pageState = buildNativePageState(summary);
  return {
    tabId: summary.tabId,
    windowId: 0,
...
function buildNativeTabInventoryEntry(summary) {
  const pageState = buildNativePageState(summary);
  return {
...
function resolveNativeTabContextTarget(tabs, target) {
  if (!target) {
    return null;
  }
...
function sortTrackedTabs(tabs, activeTabId) {
  return tabs.slice().sort((left, right) => {
    ...
    const leftTitle = normalize2(left.title || left.url || `Tab ${left.tabId}`).toLowerCase();
    const rightTitle = normalize2(right.title || right.url || `Tab ${right.tabId}`).toLowerCase();
...
function scoreCandidate(candidate, query) {
  const normalizedCandidate = normalize3(candidate);
  const normalizedQuery = normalize3(query);
...
var executor = null;
var executorInit = null;
var lastFocusedTabKey = null;
...
function nativeTabKey(tab) {
  return tab.browserTargetId ?? `tab-${tab.tabId}`;
}
function attachFocusState(tab) {
...
async function ensureLocalRelayIfNeeded() {
...
async function executeBrowserJson(code, timeoutMs = 15e3) {
...
async function collectNativeTabSummaries() {
  const summaries = await executeBrowserJson(
    `(async () => {
  ...
  return parsed.map((entry) => normalizeNativeBrowserTabSummary(entry)).filter((entry) => entry !== null);
}
function buildTrackedTabs(summaries) {
  const activeSummary = summaries.find((summary) => summary.active) ?? null;
  if (activeSummary) {
    lastFocusedTabKey = nativeTabKey(activeSummary);
  }
  return summaries.map((summary) => attachFocusState(summary)).map((summary) => buildNativeTrackedTab(summary));
}
function buildInventoryEntries(summaries) {
  return summaries.map((summary) => buildNativeTabInventoryEntry(attachFocusState(summary)));
}
function resolveTabTargetFromArgs(args) {
...
async function getInventory() {
  ...
  const entriesByTabId = new Map(entries.map((entry) => [entry.tabContext.tabId, entry]));
  const browserExecutor = await getExecutor();
...
async function focusNativeTab(target) {
  const browserExecutor = await getExecutor();
  const summaries = await collectNativeTabSummaries();
  ...
  const result = await browserExecutor.execute(focusCode, 15e3);
  if (result.isError) {
...
server.tool(
  ...
  async ({ query, limit }) => {
    ...
    const entriesByTabId = new Map(inventory.tabs.map((entry) => [entry.tabContext.tabId, entry]));
    return {
      content: [
        {
          type: "text",
...
server.tool(
  ...
  {
    tabContext: external_exports.unknown().optional(),
    tabId: external_exports.number().int().nonnegative().optional()
  },
  async (args) => {
    const inventory = await getInventory();
    if (!inventory.connected) {
    ...
    const match = resolveNativeTabContextTarget(
      inventory.tabs.map((entry2) => ({
        tabId: entry2.tabContext.tabId,
        browserTargetId: entry2.browserTargetId,
        url: entry2.tabContext.url,
        title: entry2.tabContext.title,
        readyState: entry2.pageState.readyState,
        interactiveCount: entry2.pageState.interactiveCount,
        formCount: entry2.pageState.formCount,
        visibleTextLength: entry2.pageState.visibleTextLength,
        hasSensitiveInputs: entry2.pageState.hasSensitiveInputs,
        active: entry2.active,
        capturedAt: entry2.pageState.updatedAt
      })),
      target
    );
    if (!match) {
...
await main();
//# sourceMappingURL=tab-mcp.js.map
